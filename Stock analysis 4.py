...
def calc_return(dip, buy_dates):
    ...
    # get time interval over which shares are bought
    total_time = int((buy_dates['date'].iloc[-1] - buy_dates['date'].iloc[0]).days / 365)
    ...
    end_price = sp_changes['close'].iloc[-1]
    
    # calculate total ROI and annualized ROI
    # return_pct = 100*round((current_price - avg_share_price) / avg_share_price, 3)
    return_pct = 100*((end_price / avg_share_price) - 1)
    # roi_annual = round(roi / (sp_changes['date'].iloc[-1].year - sp_changes['date'].iloc[0].year), 2)
    avg_time_holding_share = (total_time + 1) / 2
    annual_return_pct = round(100*((end_price / avg_share_price) ** (1/avg_time_holding_share) - 1), 2)
    
    # create dataframe summarizing results
    out = pd.DataFrame({'dip': dip,
                        'paid': paid, 
                        'shares': shares,
                        'buy day fraction': 100*shares/len(sp_changes),
                        'avg share price': avg_share_price, 
                        'current price': end_price, 
                        'return (%)': return_pct, 
                        'annual return (%)': annual_return_pct}, index=[0])
...
calc_return(0, sp_changes)

# # buy when discount exceeds n%
# n = 15
# discount_n = sp_changes[sp_changes['off from high'] < -n]
...
for i in np.arange(0, 56, 0.1):
    discount_n = sp_changes[sp_changes['off from high'] <= -i]
...
return_vs_dip_buy.to_csv('./data/return_vs_dip_buy_since_' + str(min_year) + '.csv', index=False)
...
figure_name = './images/sp_buy_dips_log_since_' + str(min_year) + '.png'
plt.savefig(figure_name, dpi = 250)
plt.show()
